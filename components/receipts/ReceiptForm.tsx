...
export const ReceiptForm = () => {
  ...
  // Generate receiptId only on client after hydration
  useEffect(() => {
    setReceiptId(generateDocumentNumber("receipt"));
  }, []);
  ...
  const inputCls =
    "w-full bg-[var(--bg)] border border-[var(--border)] rounded-xl px-4 py-2.5 text-sm outline-none focus:border-[var(--brand)] transition-colors";
  const labelCls = "block text-xs font-semibold text-muted mb-1.5";
  const sectionCls =
    "bg-card border border-[var(--border)] rounded-2xl p-5 space-y-4";
