...
import { generateDocumentNumber, calculateTotals, formatCurrency, cn } from "@/lib/utils";
...
const emptyItem = (): LineItem => ({ id: uuidv4(), name: "", quantity: 1, price: 0 });
...
export const DocumentForm = ({ type }: DocumentFormProps) => {
  ...
  const [form, setForm] = useState<Omit<DocumentData, "subtotal" | "taxAmount" | "total">>({
  ...
  const handleSave = async () => {
    if (!form.businessName || !form.customerName || form.items.some((i) => !i.name)) return;
    setSaving(true);
    ...
    if (logoFile) {
      ...
      if (user) {
        ...
        const { data } = await supabase.storage.from("logos").upload(path, logoFile);
        if (data) {
    ...
    try {
      ...
      router.push(type === "invoice" ? "/dashboard/invoices" : "/dashboard/receipts");
    } catch (err) {
      console.error(err);
    }
    setSaving(false);
  ...
  return (
    <div className="flex flex-col h-full">
        ...
        {(["form", "preview"] as const).map((tab) => (
          <button
            ...
            className={cn(
              ...
              activeTab === tab ? "border-b-2 border-[var(--brand)] text-[var(--brand)]" : "text-muted"
            )}
          >
            {tab}
          </button>
      ...
      <div className="flex flex-1 overflow-hidden">
        {/* Form panel */}
        <div className={cn("flex-1 overflow-y-auto p-6 scrollbar-thin", activeTab === "preview" && "hidden md:block")}>
          <div className="max-w-xl mx-auto space-y-6">
            {/* Template */}
            <div>
              <label className="block text-xs font-semibold text-muted mb-2">Template</label>
            ...
            {/* Logo */}
            <div>
              <label className="block text-xs font-semibold text-muted mb-2">Logo</label>
            ...
            <fieldset className="bg-card border border-[var(--border)] rounded-2xl p-5 space-y-3">
              <legend className="text-xs font-bold px-1 -mt-3 bg-card text-muted">Business Info</legend>
              <input
            ...
            {/* Customer info */}
            <fieldset className="bg-card border border-[var(--border)] rounded-2xl p-5 space-y-3">
              <legend className="text-xs font-bold px-1 -mt-3 bg-card text-muted">Customer Info</legend>
              <input
            ...
            <div className="grid grid-cols-2 gap-3">
              ...
              {type === "invoice" && (
                <div>
                  <label className="block text-xs font-semibold text-muted mb-1.5">Due date</label>
                  <input
                    type="date"
                    value={form.dueDate}
              ...
              <div>
                <label className="block text-xs font-semibold text-muted mb-1.5">Currency</label>
                <select
                  value={form.currency}
              ...
              <div>
                <label className="block text-xs font-semibold text-muted mb-1.5">Payment status</label>
                <select
                  value={form.paymentStatus}
                  onChange={(e) => setForm({ ...form, paymentStatus: e.target.value as DocumentData["paymentStatus"] })}
                  className="w-full bg-card border border-[var(--border)] rounded-xl px-3 py-2.5 text-sm outline-none focus:border-[var(--brand)] transition-colors"
                >
            ...
            <div className="bg-card border border-[var(--border)] rounded-2xl p-5">
              <div className="grid grid-cols-2 gap-3 mb-4">
                <div>
                  <label className="block text-xs font-semibold text-muted mb-1.5">Tax rate (%)</label>
                  <input
                    type="number"
                    min={0}
                    max={100}
                ...
                <div>
                  <label className="block text-xs font-semibold text-muted mb-1.5">Discount ({form.currency})</label>
                  <input
                    type="number"
                    min={0}
                    step={0.01}
              ...
              <div className="space-y-1.5 text-sm">
                ...
                <div className="flex justify-between font-extrabold text-base pt-2 border-t border-[var(--border)]">
                  <span>Total</span>
                  <span className="text-[var(--brand)]">{formatCurrency(total, form.currency)}</span>
                </div>
              </div>
            </div>
        ...
        <div className={cn(
          ...
          <DocumentPreview data={documentData} type={type} template={template} logoUrl={logoPreview} />
        </div>
      </div>
    </div>
  );
