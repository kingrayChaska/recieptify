...
import { ArrowLeft, Edit2, Trash2, ReceiptIcon, Save, Download, Share2, Check } from "lucide-react";
...
export default function ReceiptDetailPage() {
  ...
  const handleShare = async () => {
    ...
    if (!token) {
      ...
      await supabase.from("receipts").update({ share_token: token }).eq("id", id);
      setReceipt({ ...receipt, share_token: token });
    }
      ...
      <div className="flex h-screen bg-base">
        ...
        <div className="flex-1 flex items-center justify-center">
          <div className="text-center">
            ...
            <Link
              href="/receipts"
              className="text-[var(--brand)] text-sm mt-2 block"
            >
              Back to receipts
            </Link>
          </div>
  ...
  const color =
    CATEGORY_COLORS[receipt.category as ReceiptCategory] ?? "#9ca3af";
        ...
        <header className="sticky top-0 z-20 bg-[var(--bg)]/80 backdrop-blur border-b border-[var(--border)] px-6 py-4 flex items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            <button
              ...
              className="flex items-center gap-1.5 px-3 py-1.5 border border-[var(--border)] rounded-xl text-xs font-semibold hover:border-[var(--brand)] transition-colors"
            >
              <Download className="w-3.5 h-3.5" /> Download
            </button>
            <button
              onClick={handleShare}
              className={cn("flex items-center gap-1.5 px-3 py-1.5 border rounded-xl text-xs font-semibold transition-colors", copied ? "border-green-500 text-green-500" : "border-[var(--border)] hover:border-[var(--brand)]")}
            >
            ...
            <button
              ...
              className="flex items-center gap-1.5 px-3 py-1.5 border border-red-500/20 rounded-xl text-xs font-semibold text-red-500 hover:bg-red-500/5 transition-colors"
            >
                  ...
                  <div>
                    ...
                    <select
                      ...
                      onChange={(e) =>
                        setForm({
                          ...form,
                          category: e.target.value as ReceiptCategory,
                        })
                      }
                      className="w-full bg-[var(--bg)] border border-[var(--border)] rounded-xl px-4 py-2.5 text-sm outline-none focus:border-[var(--brand)]"
                    >
                ...
                <div className="space-y-4">
                  ...
                  <div className="grid grid-cols-2 gap-4 text-sm">
                    <div>
                      ...
                      <p className="font-extrabold text-2xl text-[var(--brand)]">
                        {formatNaira(receipt.amount ?? 0)}
                      </p>
                    </div>
                    <div>
