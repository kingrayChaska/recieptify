...
export default async function AnalyticsPage() {
  ...
  const totalRevenue = docs.reduce(
    (sum, d) => sum + ((d.data as DocumentData).total ?? 0),
    0,
  );
  const paidInvoices = invoices.filter(
    (d) => (d.data as DocumentData).paymentStatus === "paid",
  );
  const unpaidInvoices = invoices.filter(
    (d) => (d.data as DocumentData).paymentStatus === "unpaid",
  );
  ...
  docs.forEach((d) => {
    ...
    monthlyData[month] =
      (monthlyData[month] ?? 0) + ((d.data as DocumentData).total ?? 0);
  });
      ...
      <div className="p-6 space-y-6 animate-fade-in">
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
          {stats.map(({ label, value, icon: Icon, color, bg }) => (
            <div
              key={label}
              className="bg-card border border-[var(--border)] rounded-2xl p-5"
            >
              <div
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-5">
          ...
          <div className="bg-card border border-[var(--border)] rounded-2xl p-6">
            ...
            {months.length === 0 ? (
              <div className="flex items-end justify-center h-32 text-muted text-sm">
                No data yet
              </div>
            ) : (
                ...
                {months.map(([month, value]) => (
                  ...
                  >
                    ...
                    <div
                      ...
                      style={{
                        height: `${Math.max((value / maxVal) * 96, 4)}px`,
                        opacity: 0.8,
                      }}
                    />
              ...
              {[
                ...
                },
                {
                  ...
                  count: invoices.filter(
                    (d) => (d.data as DocumentData).paymentStatus === "partial",
                  ).length,
                  color: "bg-yellow-500",
                  total: Math.max(invoices.length, 1),
                },
