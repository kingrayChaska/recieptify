...
export default function RegisterPage() {
  ...
  const handleRegister = async (e: React.FormEvent) => {
    ...
    // user isn't actually logged in yet — sending them to /dashboard would
    // just bounce them straight back to /auth/login. Only redirect once a
    // real session exists.
    if (data.session) {
      ...
      <div className="min-h-screen bg-base flex items-center justify-center px-4">
        <div className="text-center animate-fade-in px-4">
          <div className="w-16 h-16 rounded-full bg-[var(--brand)]/20 flex items-center justify-center mx-auto mb-4">
            <span className="text-2xl">{needsEmailConfirm ? "📧" : "✓"}</span>
          </div>
          ...
          {needsEmailConfirm && (
            <Link
              href="/auth/login"
              className="inline-block mt-5 text-sm text-[var(--brand)] font-semibold hover:underline"
            >
              Back to login
            </Link>
          )}
        ...
        <div className="bg-card border border-[var(--border)] rounded-2xl p-6 shadow-sm">
          ...
          <div className="grid grid-cols-2 gap-3">
            <button
              ...
              className="flex items-center justify-center gap-2 border border-[var(--border)] rounded-xl py-2.5 text-sm font-medium hover:border-[var(--brand)] transition-colors disabled:opacity-50"
            >
              ...
              <span className="truncate">{oauthLoading === "google" ? "Redirecting…" : "Google"}</span>
            </button>
            <button
            ...
            >
              ...
              <span className="truncate">{oauthLoading === "github" ? "Redirecting…" : "GitHub"}</span>
            </button>
          </div>
        </div>
